-- Simple Seed Data (No Auth User Required)
-- This version skips creating an auth user and just adds data for the demo user ID

-- DEMO USER ID (this should match what getCurrentUser() returns)
DO $$
DECLARE
  ...
  betrivers_id uuid;
BEGIN
  RAISE NOTICE 'Setting up data for demo user: %', demo_user_id;
  ...
  RAISE NOTICE '═══════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE 'NOTE: RLS policies allow access without auth user';
  RAISE NOTICE 'Data is accessible via the demo user ID';
